...
/// Protocol for MediaListViewModel, enables testing with mocks
@MainActor
protocol MediaListViewModelProtocol: AnyObject, Observable {
    var viewState: ViewState<[MediaItem]> { get }
    var selectedFilter: MediaKind? { get set }
    var sortOrder: MediaListSortOrder { get set }
...
enum MediaListSortOrder: String, CaseIterable {
...
/// ViewModel for the Media List screen
/// Shows all media items with filtering and sorting
@MainActor
@Observable
final class MediaListViewModel: MediaListViewModelProtocol {
    ...
    var sortOrder: MediaListSortOrder = .recentlyAdded
    ...
    var activeMediaKinds: [MediaKind] {
        let kinds = Set(mediaItems.map { $0.kind })
